...
app.setPath(
	'userData',
	app.getPath('userData').replace('Hyperspace.app', 'HyperspaceFullNode')
) // fix path
...
export default function configManager(filepath) {
	...
	// always overwrite the datadir, because we may switch
	// between full and spv node data dirs.
	config.hsd.datadir = defaultConfig.hsd.datadir

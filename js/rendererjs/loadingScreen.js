...
const showError = (error) => {
	...
	errorLog.textContent = 'A critical error loading Hyperspace has occured: ' + error
...
const startUI = (welcomeMsg, initUI) => {
	...
	const updateSyncStatus = async function() {
		try {
			const consensusData = await Hsd.call(hsdConfig.address, {timeout: 500, url: '/consensus'})
			const gatewayData = await Hsd.call(hsdConfig.address, {timeout: 500, url: '/gateway'})
...
// exist.
const checkHyperspacePath = () => new Promise((resolve) => {
...
const unexpectedExitHandler = () => {
	try {
		const errorMsg = fs.readFileSync(Path.join(hsdConfig.datadir, 'hsd-output.log'))
...
export default async function loadingScreen(initUI) {
	...
	if (!await checkHyperspacePath()) {
		...
		dialog.showErrorBox('Hsd not found', 'Hyperspace couldn\'t locate hsd.  Please navigate to hsd.')
	...
	setTimeout(() => {
		if (overlayText.textContent === 'Loading Hyperspace...') {
			...
			document.getElementById('releasenotelink').onclick = () => {
				shell.openExternal('https://github.com/HyperspaceApp/Hyperspace/releases')
			}
		}
	}, 30000)

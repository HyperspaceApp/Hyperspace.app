...
import {
	FILES_ICON_DOCUMENT,
	FILES_TRANSFER_DOWNLOAD,
	FILES_TRANSFER_UPLOAD,
	FILES_CONTROL_OPEN_FILE_FOLDER,
...
const Transfer = ({ name, progress, status, speed, onClick, isDownload }) => {
	...
	return (
		<li
			className={`filetransfer ${isDownload ? 'is-download' : ''}`}
			onClick={onClick}
		>
			<div className="transfer-info">
				<img src={FILES_ICON_DOCUMENT} className="icon" />
				{isDownload ? (
					<img src={FILES_TRANSFER_DOWNLOAD} className="icon" />
				) : (
					<img src={FILES_TRANSFER_UPLOAD} className="icon" />
				)}

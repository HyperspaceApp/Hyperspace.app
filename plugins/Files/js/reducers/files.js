...
export default function filesReducer(state = initialState, action) {
	switch (action.type) {
		...
		case constants.DOWNLOAD_FILE:
			return state.set(
				'unreadDownloads',
				state.get('unreadDownloads').add(action.file.hyperspacepath)
			)
		case constants.UPLOAD_FILE:
			return state.set(
				'unreadUploads',
				state.get('unreadUploads').add(action.hyperspacepath)
			)
		case constants.RECEIVE_FILES: {
			...
			const workingDirectoryHyperspacepaths = workingDirectoryFiles.map(
				(file) => file.hyperspacepath
			)
			// filter out selected files that are no longer in the working directory
			const selected = state
				.get('selected')
				.filter((file) =>
					workingDirectoryHyperspacepaths.includes(file.hyperspacepath)
				)
			return state
		...
		case constants.DESELECT_FILE:
			return state.set(
				...
				state
					.get('selected')
					.filter((file) => file.hyperspacepath !== action.file.hyperspacepath)
			)

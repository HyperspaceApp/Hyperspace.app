...
export const checkHyperspacePath = () =>
	new Promise((resolve, reject) => {
		fs.stat(HyperspaceAPI.config.attr('hsc').path, (err) => {
			if (!err) {
				if (
					Path.basename(HyperspaceAPI.config.attr('hsc').path).indexOf(
						'hsc'
					) !== -1
				) {
					resolve()
...
export const initPlugin = () =>
	checkHyperspacePath().catch(() => {
		...
		HyperspaceAPI.config.attr('hsc', {
			path: Path.resolve(
				Path.dirname(HyperspaceAPI.config.attr('hsd').path),
				process.platform === 'win32' ? './hsc.exe' : './hsc'
			),
		})
		checkHyperspacePath().catch(() => {
			...
			HyperspaceAPI.showError({
				title: 'Hsc not found',
				content: "Hyperspace couldn't locate hsc. Please navigate to hsc.",
			})
			...
			if (hscPath) {
				if (Path.basename(hscPath[0]).indexOf('hsc') === -1) {
					HyperspaceAPI.showError({
						...
						content:
							'The hsc binary must be called hsc. Restart the plugin to choose a valid binary.',
					})
				} else {
				...
				HyperspaceAPI.showError({
					title: 'Hsc not found',
					content:
						'This plugin will be unusable until a proper hsc binary is found.',
				})
			}
		})
...
export const spawnCommand = function(commandStr, actions, newid) {
	...
	if (
		...
		HyperspaceAPI.config.attr('address')
	) {
		args = args.concat(['-a', HyperspaceAPI.config.attr('address')])
	}

	const hsc = child_process.spawn(HyperspaceAPI.config.attr('hsc').path, args, {
...
export const httpCommand = function(commandStr, actions, newid) {
	...
	if (args.indexOf('-a') === -1 && args.indexOf('--address') === -1) {
		if (HyperspaceAPI.config.attr('address')) {
			...
			hyperspaceAddr = url.parse(
				'http://' + HyperspaceAPI.config.attr('address')
			)
		}
	} else {
		// Parse address flag.
